...
class Icdar(imdb):
    ...
    def rotate_to_qua(self, x1, y1, x2, y2, h):
        if x1 == x2:
            if y1 > y2:
                return np.array([x1, y1, x2, y2, x1 + h, y2, x1 + h, y1])
            else:
                return np.array([x1, y1, x2, y2, x1 - h, y2, x1 - h, y1])
    ...
    def _load_icdar_rec_annotation(self, img_name):
        ...
        height = _img.size[1]
        if cfg.QUAD_MODE:
            boxes = np.zeros((num_panels, 9), dtype=np.uint16)
        else:
        ...
        for info in boxes_select:
            rotated_rec = list(map(float, info[:5]))
            coordinates = self.rotate_to_qua(*rotated_rec)
            text = info[-1]
            # print(info)
            ...
            if cfg.QUAD_MODE:
                rotated_rec = list(map(float, rotated_rec))
                boxes[cur, :] = [x1, y1, x2, y2] + rotated_rec
            else:
