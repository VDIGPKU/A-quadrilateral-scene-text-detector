...
def parse_args():
  ...
  parser.add_argument('--pkl', dest='pkl_dir',
                      help='pkl dir',
                      default='/home/liuyudong/icdar-notb',
                      type=str)
...
def gen_result_file(out_name, pkl_dir, poly=False, dataset='frame'):
  print ('pkl path is {}'.format(pkl_dir))
  results = pickle.load(open(osp.join(pkl_dir, 'detections.pkl'),'rb'),encoding='iso-8859-1')
  out_dir = '/home/liuyudong/icdar-notb/data/{}/result/{}'.format(dataset, out_name)
  print(out_dir)
  if poly:
    out_dir += '_poly'
