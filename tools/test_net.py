...
import pickle
from lib.roi_data_layer.roidb import get_imdb
from lib.roi_data_layer.roibatchLoader import TestLoader
...
if __name__ == '__main__':
    ...
    imdb = get_imdb(args.imdbval_name)
    ...
    save_name = 'faster_rcnn_epoch_{}'.format(args.epoch)
    num_images = len(imdb.image_index)
    all_boxes = [[[] for _ in xrange(num_images)]
                 for _ in xrange(imdb.num_classes)]
    tag = args.tag
    tag = tag if tag else 'default'
    output_dir = get_output_dir(imdb, tag + '/' + save_name)
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    dataset = TestLoader(imdb)
        ...
        if cfg.TEST.BBOX_REG:
            ...
            if cfg.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
                ...
                if args.class_agnostic:
                    ...
                    box_deltas = box_deltas.view(-1, 5) * torch.FloatTensor(cfg.TRAIN.BBOX_NORMALIZE_STDS_REG).cuda() \
                                 + torch.FloatTensor(cfg.TRAIN.BBOX_NORMALIZE_MEANS_REG).cuda()
                    box_deltas = box_deltas.view(1, -1, 5 * len(imdb.classes))
    ...
    if 'voc' in imdb.name:
        ...
        imdb.evaluate_detections(tag, args.epoch)
    end = time.time()
